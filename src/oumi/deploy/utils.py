...
def check_hf_model_accessibility(model_id: str) -> bool:
    ...
    """
    if not _HF_REPO_ID_RE.match(model_id):
        return True
    ...
    try:
        ...
        logger.debug(
            f"Unexpected error checking HF model '{model_id}'; "
            "assuming public.",
            exc_info=True,
        )
        return True
...
def warn_if_private_model_missing_token(
    ...
    Checks model accessibility via :func:`check_hf_model_accessibility`. If
    the model is not publicly accessible **and** neither *model_access_key*
    nor the ``HF_TOKEN`` environment variable is set, logs a warning advising
    the user to provide a token.

    Call this before any Parasail API interaction that references a HuggingFace
    model (``upload_model``, ``create_endpoint``, ``list_hardware``).

    Args:
        model_id: HuggingFace repo ID or URL.
        model_access_key: An explicit HuggingFace token passed by the caller.
            If provided (non-empty), the check is skipped.
    """
    if model_access_key:
        return
