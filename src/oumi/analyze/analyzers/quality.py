...
"""Data quality analyzer implementation and result model.
...
class DataQualityMetrics(BaseModel):
    ...
    num_consecutive_same_role: int = Field(
        default=0,
        description=(
            "Number of consecutive messages from the same role (excluding system)"
        ),
    )
    ...
    has_invalid_values: bool = Field(
        description=(
            "Whether content contains invalid serialized values"
            " like 'NaN', 'null', 'None'"
        )
    )
...
class DataQualityAnalyzer(ConversationAnalyzer[DataQualityMetrics]):
    ...
    def _check_turn_pattern(self, conversation: Conversation) -> tuple[bool, str, int]:
    ...
    def _check_truncation(
        ...
        reason = (
            ...
            else ("No proper ending punctuation" if not ends_properly else None)
        )
    ...
    def _check_tags(self, conversation: Conversation) -> tuple[bool, bool, list[str]]:
    ...
    def analyze(self, conversation: Conversation) -> DataQualityMetrics:
        ...
        if not fits_4k:
            quality_issues.append(
                f"Exceeds 4K context ({estimated_tokens} est. tokens)"
            )
