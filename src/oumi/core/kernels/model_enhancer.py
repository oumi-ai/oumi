...
def enhance_model_with_kernels(
    model: nn.Module, model_params: ModelParams
) -> nn.Module:
    ...
    if (
        ...
        and is_flash_attn3_kernel_available()
    ):
        ...
        logger.info(
            "Flash Attention 3 kernels not available or not using Flash Attention"
        )
...
def _apply_flash_attn3_kernels(model: nn.Module) -> nn.Module:
    ...
    try:
        ...
        logger.info(
            f"Successfully applied FA3 kernels to {optimized_count}/{len(attention_layers)} layers"
        )
        return model
...
def get_kernel_optimization_info(
    model: nn.Module, model_params: ModelParams
) -> dict[str, Any]:
