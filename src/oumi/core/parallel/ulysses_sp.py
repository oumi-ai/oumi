...
class UlyssesSPAttentionHF:
    ...
    def register_with_transformers(model: PreTrainedModel, sequence_parallel_size: int):
        ...
        if not dist.is_initialized():
            logger.warning(
                "Distributed training not initialized, skipping Ulysses SP registration"
            )
            return
            ...
            if any(layer_name in name for layer_name in layer_names):
                if hasattr(module, "forward"):
                    ...
                    module.forward = create_patched_forward(
                        original_forward, sequence_parallel_size
...
def _ulysses_sp_attention_forward(
    ...
    **kwargs,
    ...
    dist.all_gather_into_tensor(gathered_hidden_states.view(-1), hidden_states.view(-1))
...
def is_ulysses_sp_available() -> bool:
    ...
    return (
        dist.is_available()
        and importlib.util.find_spec("torch.distributed") is not None
    )


def setup_ulysses_sp(
    model: PreTrainedModel, dataloader: DataLoader, sequence_parallel_size: int
    ...
    if not is_ulysses_sp_available():
        logger.warning(
            "Ulysses SP not available, returning original model and dataloader"
        )
    ...
    adapted_dataloader = UlyssesSPDataLoaderAdapter(
        dataloader=dataloader, sequence_parallel_size=sequence_parallel_size
    )
