...
class BaseMapDataset(MapDataPipe, Sized, ABC):
    ...
    def __len__(self) -> int:
        ...
        assert self._raw_hf_data is not None
    ...
    def data(self) -> pd.DataFrame:
        ...
        if self._pandas_cache is None:
            self._pandas_cache = self._raw_hf_data.to_pandas()
    ...
    def to_hf(
        ...
        Args:
            ...
            use_native_map: Use HF `.map()` for faster conversion.
    ...
    def _to_hf_native(
        ...
        result = self._raw_hf_data.map(
            self._transform_batch,
            batched=True,
            batch_size=1000,
            num_proc=(num_proc if num_proc > 1 else None),
            remove_columns=self._raw_hf_data.column_names,
    ...
    def _to_hf_generator(
        ...
        """Legacy conversion using generator."""
    ...
    def _load_raw_hf_dataset(self) -> datasets.Dataset:
        """Loads the dataset as HF Dataset."""
        if (
            self.dataset_path
