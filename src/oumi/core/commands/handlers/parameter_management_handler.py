...
class ParameterManagementHandler(BaseCommandHandler):
    ...
    def _handle_set(self, command: ParsedCommand) -> CommandResult:
        ...
        if changes_made:
            ...
            return CommandResult(
                success=False,
                message=" | ".join(messages) if messages else "No parameters were updated",
                should_continue=False,
            )
    ...
    def _update_parameter(self, param: str, value: str) -> Tuple[bool, str]:
        ...
        valid_params = {
            ...
            'sampling', 'seed', 'frequency_penalty', 'presence_penalty',
            'min_p', 'num_beams'
        }
        
        if param not in valid_params:
            return False, f"Unknown parameter '{param}'. Valid parameters: {', '.join(sorted(valid_params))}"
    ...
    def _parse_parameter_value(self, param: str, value: str) -> Any:
        ...
        # Float parameters
        elif param in ['temperature', 'top_p', 'frequency_penalty', 'presence_penalty', 'min_p']:
            try:
                return float(value)
            except ValueError:
