...
class ImportUtilities:
    ...
    def import_conversation(self, file_path: str) -> Tuple[bool, str, List[Dict[str, Any]]]:
    ...
    def _import_from_json(self, file_path: str) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                if "conversation" in data:
                    ...
                    if "role" in data and "content" in data:
                        ...
                        return False, "Unable to find conversation data in JSON file", []
            else:
            ...
            return True, f"Successfully imported {len(valid_messages)} messages from JSON", valid_messages
    ...
    def _import_from_csv(self, file_path: str) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8', newline='') as f:
                ...
                if not role_col or not content_col:
                    return False, "CSV must contain 'role' and 'content' columns (or similar)", []
            ...
            return True, f"Successfully imported {len(messages)} messages from CSV", messages
            
        except Exception as e:
            return False, f"Error reading CSV file: {str(e)}", []
    
    def _import_from_excel(self, file_path: str) -> Tuple[bool, str, List[Dict[str, Any]]]:
        """Import conversation from Excel format."""
        try:
            import pandas as pd
        except ImportError:
            return False, "Excel import requires pandas. Install with: pip install pandas openpyxl", []
        ...
        try:
            ...
            if not role_col or not content_col:
                return False, "Excel file must contain 'role' and 'content' columns (or similar)", []
            ...
            return True, f"Successfully imported {len(messages)} messages from Excel", messages
            
        except Exception as e:
            return False, f"Error reading Excel file: {str(e)}", []
    
    def _import_from_markdown(self, file_path: str) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            for i in range(1, len(parts), 3):  # Skip first empty part, then take header+content pairs
                ...
                role_match = re.search(r'(user|assistant|system|human|ai|bot)', header_text.lower())
                if role_match:
                    ...
                    current_role = role
                    current_content = [section_content.strip()] if section_content.strip() else []
                elif current_role:
            ...
            return True, f"Successfully imported {len(messages)} messages from Markdown", messages
            
        except Exception as e:
            return False, f"Error reading Markdown file: {str(e)}", []
    
    def _import_from_text(self, file_path: str) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            if matches:
                ...
                for part in parts:
                    ...
                    role_match = re.match(r'^(user|assistant|system|human|ai|bot)[:\\s]+(.+)', part, re.IGNORECASE | re.DOTALL)
                    if role_match:
            ...
            return True, f"Successfully imported {len(messages)} messages from text", messages
