...
class ConversationBranchManager:
    ...
    def create_branch(
        ...
        if len(self.branches) >= self.MAX_BRANCHES:
            return False, f"Maximum number of branches ({self.MAX_BRANCHES}) reached", None
        ...
        new_branch = ConversationBranch(
            ...
            conversation_history=copy.deepcopy(source_branch.conversation_history[:branch_point])
        )
        
        self.branches[new_id] = new_branch
        return True, f"Created branch '{new_branch.name}' from '{source_branch.name}'", new_branch
    
    def switch_branch(self, branch_id: str) -> Tuple[bool, str, Optional[ConversationBranch]]:
    ...
    def list_branches(self) -> List[Dict[str, any]]:
        ...
        branches_info.sort(
            key=lambda x: (not x["is_current"], x["last_active"]), 
            reverse=True
        )
