...
class CostAnalyzer(SampleAnalyzer):
    ...
    Note:
        This analyzer depends on token count columns (e.g., `*_token_count`)
        which are generated by the `length` analyzer. Ensure the `length`
        analyzer runs before this analyzer in the pipeline. If no token count
        columns are found, this analyzer will return the input DataFrame unchanged.
