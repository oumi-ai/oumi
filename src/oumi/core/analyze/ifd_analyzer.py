...
class IFDAnalyzer(SampleAnalyzer):
    ...
    def get_output_schema(
        ...
        aid: str = analyzer_id or getattr(self, "analyzer_id", "ifd")

        # IFD analyzer uses response columns, typically text_content
        ...
        for column in text_columns:
            ...
            output_schema[col_name] = {
                ...
                "description": "IFD score (lower = better instruction following)",
            }
            col_name = make_analyzer_column_name(column, aid, "ppl_with_instruction")
            output_schema[col_name] = {
                "type": ColumnType.FLOAT,
                "content_type": ContentType.NUMERIC,
                "description": "Perplexity with instruction context",
            }
            col_name = make_analyzer_column_name(
                column, aid, "ppl_without_instruction"
            )
            output_schema[col_name] = {
                "type": ColumnType.FLOAT,
                "content_type": ContentType.NUMERIC,
            ...
            col_name = make_analyzer_column_name(column, aid, "response_loss")
            output_schema[col_name] = {
                "type": ColumnType.FLOAT,
                "content_type": ContentType.NUMERIC,
                "description": "Response loss value",
            }
