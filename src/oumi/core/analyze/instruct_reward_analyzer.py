...
class InstructRewardAnalyzer(SampleAnalyzer):
    ...
    def get_output_schema(
        ...
        aid: str = analyzer_id or getattr(self, "analyzer_id", "instruct_reward")
        ...
        for column in text_columns:
            ...
            output_schema[col_name] = {
                ...
                "description": "Overall instruct reward score (0-5 scale)",
            }
            col_name = make_analyzer_column_name(column, aid, "tier")
            output_schema[col_name] = {
                "type": ColumnType.STRING,
                "content_type": ContentType.CATEGORICAL,
                "description": "Quality tier (poor, fair, good, excellent)",
            }

            if self.include_component_scores:
                col_name = make_analyzer_column_name(column, aid, "helpfulness")
                output_schema[col_name] = {
                    "type": ColumnType.FLOAT,
                    "content_type": ContentType.NUMERIC,
                    "description": "Helpfulness score (0-1)",
                }
                col_name = make_analyzer_column_name(column, aid, "completeness")
                output_schema[col_name] = {
                    "type": ColumnType.FLOAT,
                    "content_type": ContentType.NUMERIC,
                    "description": "Completeness score (0-1)",
                }
                col_name = make_analyzer_column_name(column, aid, "clarity")
                output_schema[col_name] = {
                    "type": ColumnType.FLOAT,
                    "content_type": ContentType.NUMERIC,
