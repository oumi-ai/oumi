...
class ContextWindowManager:
    ...
    def __init__(self, max_context_length: int = 4096, model_name: str = "gpt-3.5-turbo"):
    ...
    def calculate_budget(self, conversation_length: int = 0) -> ContextBudget:
        ...
        history_reserve = max(
            int(self.max_context_length * 0.2),
            conversation_length
        )
    ...
    def suggest_chunking_strategy(self, total_tokens: int, available_tokens: int) -> dict:
        ...
        chunk_size = max(available_tokens - 100, available_tokens // 2)  # Leave some margin
