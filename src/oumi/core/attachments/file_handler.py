...
class FileHandler:
    ...
    CODE_EXTENSIONS = {'.py', '.js', '.ts', '.html', '.css', '.java', '.cpp', '.c', '.h', 
                      '.go', '.rs', '.php', '.rb', '.swift', '.kt', '.scala', '.sh', '.yaml', '.yml'}
    ...
    def attach_file(self, file_path: str, conversation_tokens: int = 0) -> AttachmentResult:
    ...
    def _determine_strategy(self, file_type: FileType, size_bytes: int) -> ProcessingStrategy:
    ...
    def _process_file(self, file_info: FileInfo, budget) -> AttachmentResult:
        ...
        try:
            ...
            elif file_info.file_type in [FileType.TEXT, FileType.MARKDOWN, FileType.CODE]:
...
"""
    ...
    def _process_pdf(self, file_info: FileInfo, budget) -> AttachmentResult:
        ...
        if file_info.processing_strategy == ProcessingStrategy.PREVIEW_ONLY:
            text_content += "⚠️ PDF is too large for full processing. Showing file info only.\n\n"
    ...
    def _process_text_file(self, file_info: FileInfo, budget) -> AttachmentResult:
        ...
        if file_info.processing_strategy == ProcessingStrategy.FULL_CONTENT and token_estimate.fits_in_budget:
            processed_content = self._format_text_content(file_info, content)
            context_info = f"Full content included ({token_estimate.estimated_tokens:,} tokens)"
            
        elif file_info.processing_strategy == ProcessingStrategy.SUMMARIZED or not token_estimate.fits_in_budget:
            # Truncate to fit budget
            truncated_content = self.context_manager.truncate_content(content, budget.available_for_content)
    ...
    def _process_json(self, file_info: FileInfo, budget) -> AttachmentResult:
        ...
        try:
            ...
            token_estimate = self.context_manager.check_content_fit(formatted_json, budget)
            
            if token_estimate.fits_in_budget:
                processed_content = self._format_json_content(file_info, formatted_json)
                context_info = f"Full JSON included ({token_estimate.estimated_tokens:,} tokens)"
                warning_message = None
            else:
                ...
                processed_content += "Ask about specific keys or sections you're interested in."
    ...
    def _format_text_content(self, file_info: FileInfo, content: str, is_preview: bool = False) -> str:
    ...
    def _create_error_result(self, file_path: str, error_message: str) -> AttachmentResult:
