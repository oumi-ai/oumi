...
class ConversationSynthesizer:
    ...
    def _run_tool_call(self, tool_call: ToolCall) -> Message:
        ...
        Returns a ``Role.TOOL`` message keyed to ``tool_call.id``. Any failure
        — malformed JSON args, unknown tool, env exception — is captured as an
        ``{"error": ...}`` JSON payload so the loop can keep running and the
        model can recover (or give up) gracefully.
        """
    ...
    def _has_empty_messages(self, conversation: Conversation) -> bool:
        ...
        Assistant messages that carry ``tool_calls`` are also excluded: their
        ``content`` is allowed to be ``None`` per the OpenAI tool wire format.
    ...
    def _run_assistant_turn_native(
        self,
        prompts: list[Conversation],
        sample_indices: list[int],
        ...
        while True:
            ...
            for idx, result in zip(active, results):
                ...
                tool_calls = (
                    assistant_msg.tool_calls if assistant_msg is not None else None
                )
                if tool_calls:
                    staging[idx].append(
                        Message(
