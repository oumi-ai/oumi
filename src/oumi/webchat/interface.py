...
class WebChatInterface:
    ...
    def __init__(
        self, config: InferenceConfig, server_url: str = "http://localhost:9000"
    ):
    ...
    def create_interface(self) -> gr.Blocks:
        ...
        with gr.Blocks(
            title="Oumi WebChat", css=custom_css, theme=gr.themes.Soft()
        ) as interface:
            # Header
            with gr.Row():
                ...
                # Model info display
                model_info = gr.HTML(
                    value=self._get_model_info_html(), elem_classes=["model-info"]
                )
            ...
            with gr.Row():
                ...
                with gr.Column(scale=2):
                    ...
                    with gr.Row():
                        clear_btn = gr.Button(
                            "Clear", size="sm", elem_classes=["command-button"]
                        )
                        delete_btn = gr.Button(
                            "Delete Last", size="sm", elem_classes=["command-button"]
                        )
                        regen_btn = gr.Button(
                            "Regenerate", size="sm", elem_classes=["command-button"]
                        )
                    ...
                    with gr.Row():
                        ...
                        export_btn = gr.Button(
                            "Export", size="sm", elem_classes=["command-button"]
                        )
                        help_btn = gr.Button(
                            "Help", size="sm", elem_classes=["command-button"]
                        )
                ...
                with gr.Column(scale=1):
                    ...
                    with gr.Accordion("Settings", open=False):
                        ...
                        max_tokens_slider = gr.Slider(
                            ...
                            maximum=4000,
                            value=getattr(
                                self.config.generation, "max_new_tokens", 2048
                            ),
                            step=100,
                            label="Max Tokens",
                        )
                        ...
                        model_selector = gr.Dropdown(
                            ...
                            value=getattr(
                                self.config.model, "model_name", "Current Model"
                            ),
                            label="Model",
                            interactive=True,
                        )
            ...
            session_state = gr.State(
                {"session_id": self.session_id, "conversation": []}
            )
            ...
            def handle_send_message(
                message: str, history: list, state: dict
            ) -> tuple[list, str, dict]:
                    ...
                    try:
                        ...
                        response = requests.post(
                            ...
                            json={
                                ...
                                "max_tokens": getattr(
                                    self.config.generation, "max_new_tokens", 100
                                ),
                                "temperature": getattr(
            ...
            def switch_branch(
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success"):
                        ...
                        state["conversation"] = history
                        state["current_branch"] = data.get(
                            "current_branch", branch_name
                        )

                        return (
                            history,
                            self._update_branch_tree(state["session_id"]),
                            state,
                        )
            ...
            def handle_file_attachment(files: list, state: dict) -> tuple[str, dict]:
                ...
                for file in files:
                    response = self._execute_command(
                        f'/attach("{file.name}")', state["session_id"]
                    )
                    if response.get("success"):
            ...
            def handle_delete_command(state):
                ...
                self._sync_conversation_to_backend(
                    state["session_id"], state.get("conversation", [])
                )
                result, updated_state = execute_command("/delete()", state)
                # Get updated conversation from backend
                updated_conversation = self._get_conversation_from_backend(
                    state["session_id"]
                )
                            ...
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    if (
                                        ...
                                        new_response = result_data["choices"][0][
                                            "message"
                                        ]["content"]
                                ...
                                conversation.append(
                                    {
                                        "role": "assistant",
                                        "content": f"Error regenerating: {str(e)}",
                                    }
                                )
                                updated_state["conversation"] = conversation
                                return conversation, updated_state
            ...
            def handle_export_command(state):
                ...
                if result and "success" in str(result) and "failed" not in str(result):
                    export_msg = {
                        "role": "assistant",
                        "content": f"✅ **Exported**: {result}",
                    }
                else:
                    export_msg = {
                        "role": "assistant",
                        "content": f"❌ **Export Failed**: {result}",
                    }
            ...
            refresh_monitor_btn.click(refresh_system_monitor, outputs=[system_monitor])

            # Settings panel functionality
            def update_temperature(value, state):
                response = self._execute_command(
                    f"/set(temperature={value})", state["session_id"]
                )
                if response.get("success"):
                    return f"✅ Temperature updated to {value}", state
                else:
                    return (
                        f"❌ Failed to update temperature: {response.get('message', 'Unknown error')}",
                        state,
                    )

            def update_max_tokens(value, state):
                response = self._execute_command(
                    f"/set(max_tokens={int(value)})", state["session_id"]
                )
                if response.get("success"):
                    return f"✅ Max tokens updated to {int(value)}", state
                else:
                    return (
                        f"❌ Failed to update max tokens: {response.get('message', 'Unknown error')}",
                        state,
                    )
            ...
            def update_model(model_name, state):
                ...
                response = self._execute_command(
                    f"/swap({model_name})", state["session_id"]
                )
                if response.get("success"):
                    ...
                    return (
                        f"❌ Failed to switch model: {response.get('message', 'Unknown error')}",
                        state,
                        current_chatbot,
                        current_model_info,
                    )
    ...
    def _execute_command(self, command: str, session_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={"session_id": session_id, "command": cmd_name, "args": args},
            )
    ...
    def _get_model_info_html(self) -> str:
        ...
        engine = (
            str(self.config.engine) if hasattr(self.config, "engine") else "Unknown"
        )
    ...
    def _get_system_monitor_html(self) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                context_info = (
                    f"{context_used}/{context_max} tokens ({context_percent:.1f}%)"
                )
    ...
    def _sync_conversation_to_backend(self, session_id: str, conversation: list):
        ...
        try:
            ...
            response = requests.post(
                ...
                json={"session_id": session_id, "conversation": conversation},
                timeout=10,
            )
...
def launch_webchat(
    ...
    print("🌐 Launching Oumi WebChat interface...")
