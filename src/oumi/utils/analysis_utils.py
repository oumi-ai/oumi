...
def save_results(
    ...
    if save_format == "json":
        ...
        _save_results_as_csv(results, output_file)
    elif save_format == "parquet":
        _save_results_as_parquet(results, output_file)
    else:
...
def _save_results_as_csv(results: dict[str, Any], output_file: Path):
    """Save results in CSV format."""
    with open(output_file, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Metric", "Value"])
        writer.writerow(["dataset_name", results.get("dataset_name", "Unknown")])
        ...
        if "sample_level_results" in results and "aggregation_results" in results:
            ...
            writer.writerow(["total_messages", sample_results.get("total_messages", 0)])
            ...
            if "conversation_stats" in aggregation_results:
                ...
                writer.writerow(["mean_turns", f"{stats.get('mean_turns', 0):.2f}"])
            ...
            writer.writerow(["total_messages", results.get("total_messages", 0)])
...
def _save_results_as_parquet(results: dict[str, Any], output_file: Path):
...
def compute_sample_level_analysis(
    dataset: BaseMapDataset, config: AnalyzerConfig
) -> dict[str, Any]:
    ...
    for conv_idx in range(conversations_to_analyze):
        ...
        for msg_idx, message in enumerate(conversation.messages):
            ...
            message_data.update(compute_message_metrics(text_content, config))
    ...
    sample_results = {
        ...
        "message_level_stats": compute_message_level_stats(messages_data),
    }
...
def compute_message_metrics(
    ...
    if sample_metrics.length.enabled:
        try:
            length_metrics = compute_length_metrics(text_content, sample_metrics.length)
    ...
    if sample_metrics.safety.enabled:
        try:
            safety_metrics = compute_safety_metrics(text_content, sample_metrics.safety)
...
def compute_language_metrics(text_content: str, language_config) -> dict[str, Any]:
...
def compute_length_metrics(text_content: str, length_config) -> dict[str, Any]:
...
def compute_safety_metrics(text_content: str, safety_config) -> dict[str, Any]:
...
def compute_message_level_stats(messages_data: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for metric_name in metric_names:
        ...
        if all(isinstance(v, (int, float)) for v in values):
            ...
            if numeric_values:
                stats[f"avg_{metric_name}"] = sum(numeric_values) / len(numeric_values)
...
def compute_aggregation_analysis(
    sample_results: dict[str, Any], config: AnalyzerConfig
) -> dict[str, Any]:
    """Perform aggregation analysis using the intermediate results."""
    verbose = config.verbose

    if verbose:
        logger.info(f"Starting aggregation analysis of dataset: {config.input.name}")
    ...
    agg_metrics = config.aggregation_metrics
    conv_agg_metrics = config.conversation_aggregation_metrics
    ...
    if (
        ...
        and hasattr(config, "outputs")
        and config.outputs.conversation_level_output
    ):
        conversation_level_data = list(conversation_stats.values())
        conversation_level_filename = generate_timestamped_filename(
            config.outputs.conversation_level_output,
            config.outputs.save_format,
        )
        conversation_level_path = (
            Path(config.outputs.path) / conversation_level_filename
        )
