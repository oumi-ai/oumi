...
class DocPanel(Static):
    ...
    def set_doc(
        self, title: str, docstring: str, value: Any = None, is_default: bool = False
    ) -> None:
    ...
    def _refresh_content(self) -> None:
        ...
        if hasattr(self, "_value") and self._value is not None:
            value_label = "[bold]Default Value:[/bold]" if getattr(self, "_is_default", False) else "[bold]Current Value:[/bold]"
            parts.append(f"{value_label} {_format_value(self._value)}\n\n")

        if getattr(self, "_is_default", False):
            parts.append("[dim italic]This field is using its default value.[/dim italic]\n\n")
...
class ConfigViewerApp(App):
    ...
    def _load_schema_async(self) -> None:
        ...
        if config_class:
            ...
            # Load docstrings for each section class
            for section_name in self.config_data.keys():
                section_cls = _get_section_class(section_name)
                if section_cls:
                    docstrings[section_name] = _extract_field_docstrings(section_cls)
            ...
            self.call_from_thread(
                self._on_schema_loaded, config_class, docstrings, default_config, merged_config
            )
    ...
    def _on_schema_loaded(
        ...
        self.notify("Schema loaded - press 'd' to show defaults", timeout=3)

    def _load_docstrings(self, cls: type) -> None:
        """Load docstrings for a config class and its nested classes."""
        # Load top-level docstrings
        self.field_docstrings[""] = _extract_field_docstrings(cls)

        # Load docstrings for each section class
        for section_name in self.config_data.keys():
            section_cls = _get_section_class(section_name)
            if section_cls:
                self.field_docstrings[section_name] = _extract_field_docstrings(
                    section_cls
                )
    ...
    def _build_tree(self) -> None:
        ...
        config_type = (
            self.config_class.__name__ if self.config_class else "Config"
        )
    ...
    def _add_nodes(
        self, parent: TreeNode, data: Any, path: str, key: str = ""
    ) -> None:
        """Recursively add nodes to the tree."""
        if isinstance(data, dict):
            for k, v in data.items():
                child_path = f"{path}.{k}" if path else k
                is_default = self.showing_defaults and not self._is_from_yaml(child_path)
                ...
                if isinstance(v, dict):
                    ...
                    label = f"[{key_style}]{k}[/{key_style}] [dim]({len(v)} items)[/dim]"
                    if is_default:
                        label += " [dim italic](default)[/dim italic]"
                    node = parent.add(label, expand=False)
                    node.data = {
    ...
    def on_tree_node_selected(self, event: Tree.NodeSelected) -> None:
        ...
        # Get the section name (first part of path)
        path = data.get("path", "")
        ...
        # First try the section-specific docstrings
        if section and section in self.field_docstrings:
            docstring = self.field_docstrings[section].get(key)

        # Fall back to top-level docstrings
        if not docstring and key in self.field_docstrings.get("", {}):
            docstring = self.field_docstrings[""].get(key)

        # Try to get docstring from nested class if it's a dict value
        if not docstring and isinstance(value, dict):
            # Check if this key has a corresponding param class
    ...
    def action_toggle_defaults(self) -> None:
        ...
        if not self.config_class:
            self.notify("No config schema detected - cannot show defaults", severity="warning")
            return
    ...
    def action_toggle_raw(self) -> None:
        ...
        if self.showing_raw:
            doc_panel.set_doc("Raw YAML", f"```yaml\n{self.raw_content}\n```")
        else:
            doc_panel.set_doc(
                "Documentation",
