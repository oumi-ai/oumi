    ...
    def load_model(self):
        ...
        if self.vision_tower_name.lower() == "maws/mae-h-14":
            self.vision_tower = torch.hub.load(
                "facebookresearch/maws", model="vit_h14_mae"
            )

        elif self.vision_tower_name.lower() == "maws/maws-h-14":
            self.vision_tower = torch.hub.load(
                "facebookresearch/maws", model="vit_h14_maws"
            )
        ...
        # res = 224
            ...
            self.vision_tower = torch.hub.load(
                "facebookresearch/maws", model="vit_l16_maws"
            )

        elif self.vision_tower_name.lower() == "maws/mae-l-16":
            self.vision_tower = torch.hub.load(
                "facebookresearch/maws", model="vit_l16_mae"
            )
