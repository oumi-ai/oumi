        ...
        if hasattr(config, "mm_vision_tower_aux_list"):
            ...
            if projector_type == "sva":
                ...
                for query_group_i in range(num_query_group):
                    ...
                    setattr(
                        self,
                        f"vision_sampler_{query_group_i}",
                        VisionTokenSampler(
                            vision_hidden_size,
                            vision_hidden_size,
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            if self.config.mm_projector_type == "sva":
                for aux_i in range(len(vision_tower_aux_list)):
                    getattr(self, f"mm_projector_aux_{aux_i}").load_state_dict(
                        get_w(mm_projector_weights, f"mm_projector_aux_{aux_i}"),
                        strict=True,
                    )

                for query_group_i in range(num_query_group):
                    getattr(self, f"vision_sampler_{query_group_i}").load_state_dict(
                        get_w(
        ...
        if self.get_model().config.mm_projector_type == "sva":
            ...
            for aux_i in range(len(vision_tower_aux_list)):
                ...
                image_aux_features = getattr(
                    self.get_model(), f"mm_projector_aux_{aux_i}"
            ...
            for query_group_i, query_num in enumerate(query_num_list):
                ...
                query_features_i = getattr(
                    self.get_model(), f"vision_sampler_{query_group_i}"
                )(
