...
class MoCoVisionTower(BaseVisionTower):
    ...
    def load_model(self, device_map=None):
        ...
        logger.info(
            f"Loaded MoCo model: {self.vision_tower_name} with hidden size: {self._hidden_size}, image size: {self._image_size}, patch size: {self._patch_size}"
        )
    ...
    def _forward(self, images):
        with torch.set_grad_enabled(self.unfreeze_mm_vision_tower):
            image_feature_outs = self.vision_tower.forward_features(
