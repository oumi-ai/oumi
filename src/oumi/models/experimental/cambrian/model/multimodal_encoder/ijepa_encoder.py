...
from .ijepa.vision_transformer import vit_giant, vit_huge
...
class IJepaVisionTower(BaseVisionTower):
    ...
    def load_model(self, device_map=None):
        ...
        if self.vision_tower_name.lower() == "ijepa-vit-h-14":
            ...
            local_path = (
                "/mnt/disks/storage/vision_ckpts/ijepa/IN22K-vit.h.14-900e.pth.tar"
            )
            ...
            local_path = (
                "/mnt/disks/storage/vision_ckpts/ijepa/IN22K-vit.g.16-600e.pth.tar"
            )
        ...
        if os.path.exists(local_path):
            logger.info(
                f"Loading `{self.vision_tower_name}` from local path: {local_path}"
            )
        ...
        logger.info(
            f"Loaded IJepa model: {self.vision_tower_name} with hidden size: {self._hidden_size}, image size: {self._image_size}, patch size: {self._patch_size}\nSetting requires_grad: {self.unfreeze_mm_vision_tower}"
        )
        self.is_loaded = True
