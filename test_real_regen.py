...
def debug_conversation_state():
    ...
    conversation_history = [
        ...
        {"role": "assistant", "content": "Kermit the Frog would be an excellent president!"}
    ]
    ...
    print(f"Regen result: success={result.success}, override='{result.user_input_override}'")
    ...
    for i in range(1, len(test_conversation)):
        if test_conversation[i-1]['role'] == test_conversation[i]['role']:
            violations.append(f"Messages {i-1} and {i} both have role '{test_conversation[i]['role']}'")
