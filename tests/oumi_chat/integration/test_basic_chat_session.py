...
class TestBasicChatSession:
    ...
    def test_error_handling_invalid_input(self, chat_session):
        ...
        # Test empty message - some implementations may accept empty messages
        result = chat_session.send_message("")
        assert isinstance(result, CommandResult)
        # Accept either success or failure for empty messages
    ...
    def test_inference_engine_error_recovery(self, chat_session):
        ...
        # Test that session can handle potential engine issues gracefully
        result = chat_session.send_message("Test message")

        # Should return a valid result regardless of underlying engine behavior
        assert isinstance(result, CommandResult)
    ...
    def test_session_state_isolation(self, chat_session):
        ...
        # Sessions should have different conversation IDs or be None
        if conv1 and conv2:
...
class TestChatSessionWithCommands:
    ...
    def test_clear_command_in_session(self, chat_session):
        ...
        messages_before = len(conv_before.messages) if conv_before else 0
        ...
        if result.success:
            ...
            # If clear worked, should have fewer or same messages
