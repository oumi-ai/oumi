...
class TestDSParamsOmegaConf:
    ...
    def test_nested_dataclass_instantiation(self):
        ...
        offload_config = OffloadConfig(
            device=DeepSpeedOffloadDevice.CPU,
            pin_memory=True,
            buffer_count=4
        )
        params = DSParams(
    ...
    def test_validation_in_post_init(self):
        ...
        with pytest.raises(ValueError, match="Parameter offloading is only supported with ZeRO stage 3"):
            params = DSParams(
        ...
        with pytest.raises(ValueError, match="Optimizer offloading requires ZeRO stage 2 or 3"):
            params = DSParams(
