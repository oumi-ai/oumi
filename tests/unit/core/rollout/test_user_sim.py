...
def test_build_user_turn_prompt_shape():
    ...
    state = RolloutState(
        persona="You are Jane, a customer.",
        max_turns=6,
        goal="Get a refund.",
        turn_idx=2,
    )
    ...
    assert DONE_SENTINEL in turn_info
...
def test_next_user_turn_normal():
    ...
    done, text = next_user_turn(
        state, [{"role": "assistant", "content": "hi"}], lambda c: "  my reply  "
    )
    assert (done, text) == (False, "my reply")
    assert state.turn_idx == 1


def test_next_user_turn_sentinel_ends():
    state = RolloutState(persona="p", max_turns=5)
    done, text = next_user_turn(state, [], lambda c: f"thanks {DONE_SENTINEL}")
    assert done is True
    assert text == "thanks"
    assert state.turn_idx == 1
...
def test_next_user_turn_produces_exactly_max_turns():
    ...
    done, text = next_user_turn(state, [], lambda c: "only turn")
    assert (done, text) == (False, "only turn")

    done, text = next_user_turn(state, [], lambda c: "should not be used")
