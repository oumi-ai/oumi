...
def test_simulated_user_tokens_are_masked_zero():
    ...
    state = _run_sim_turn(loop, data, (False, "and my refund?"))
...
def test_boundary_invariant_holds_after_append():
    loop, data = _loop(), _agent_data()
    _run_sim_turn(loop, data, (False, "and my refund?"))
...
def test_simulator_turn_cap_terminates_without_counting_a_turn():
    loop, data = _loop(max_turns=1), _agent_data()
    loop._sim.turn_idx = 1
...
def test_done_sentinel_terminates_without_tokenizing():
    ...
    state = _run_sim_turn(loop, data, (True, ""))
...
def test_overlong_turn_terminates_without_mutating():
    ...
    before_messages = list(data.messages)
    state = _run_sim_turn(loop, data, (False, "x"))

    assert state is AgentState.TERMINATED
    assert data.prompt_ids == before_ids
    assert data.response_mask == before_mask
    assert data.messages == before_messages
...
def test_turn_caps_prevent_simulated_user_turn(cap_attr, counter_attr):
    ...
    assert loop._hard_cap_hit(data, ignore_termination=False) is True
...
def test_no_caps_hit_allows_simulated_user_turn():
    ...
    assert loop._hard_cap_hit(data, ignore_termination=False) is False
...
def test_tool_turn_then_simulated_user_turn_interleave():
    ...
    loop.loop.run_until_complete(
        loop._append_environment_turn(data, {"role": "tool", "content": "status=late"})
    )
    ...
    state = _run_sim_turn(loop, data, (False, "when will it arrive?"))
