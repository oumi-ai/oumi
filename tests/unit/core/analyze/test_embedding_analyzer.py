...
class TestEmbeddingAnalyzerInit:
    ...
    def test_invalid_duplicate_threshold_raises_error(self):
        """Test that invalid duplicate threshold raises ValueError."""
        from oumi.core.analyze.embedding_analyzer import EmbeddingAnalyzer

        with pytest.raises(ValueError, match="duplicate_threshold must be in range"):
            EmbeddingAnalyzer(duplicate_threshold=1.5)

        with pytest.raises(ValueError, match="duplicate_threshold must be in range"):
            EmbeddingAnalyzer(duplicate_threshold=-0.1)

    def test_invalid_fuzzy_threshold_raises_error(self):
        """Test that invalid fuzzy threshold raises ValueError."""
        from oumi.core.analyze.embedding_analyzer import EmbeddingAnalyzer

        with pytest.raises(ValueError, match="fuzzy_threshold must be in range"):
            EmbeddingAnalyzer(fuzzy_threshold=1.5)

    def test_invalid_duplicate_scope_raises_error(self):
        """Test that invalid duplicate scope raises ValueError."""
        from oumi.core.analyze.embedding_analyzer import EmbeddingAnalyzer
...
class TestEmbeddingAnalyzerWithMocks:
    ...
    def test_compute_embeddings(self, mock_model):
        ...
        with patch(
            "oumi.core.analyze.embedding_base_analyzer.EmbeddingBasedAnalyzer._get_model"
        ) as mock_get:
    ...
    def test_analyze_sample_with_duplicates(self, mock_model):
        ...
        with patch(
            ...
            assert "text_content_embedding_duplicate_group" in generated_schema
            assert "text_content_embedding_has_semantic_duplicate" in generated_schema
    ...
    def test_analyze_sample_with_clustering(self, mock_model):
        ...
        with patch(
            ...
            assert "text_content_embedding_cluster" in generated_schema
    ...
    def test_analyze_sample_returns_tuple(self, mock_model):
        ...
        with patch(
            ...
            df = pd.DataFrame({"text_content": ["a", "b", "c"]})
...
class TestRoleAwareDuplicateDetection:
    ...
    def test_scope_all_compares_all_messages(self, mock_model):
        ...
        with patch(
            ...
            result_df, _ = analyzer.analyze_sample(df, self._get_schema(with_role=True))
...
class TestEmptyTextHandling:
    """Tests for handling empty texts."""
    ...
    def mock_model(self):
        ...
        model = MagicMock()
        model.get_sentence_embedding_dimension.return_value = 384
        model.encode.return_value = np.random.randn(3, 384)
        return model

    def test_empty_dataframe(self, mock_model):
