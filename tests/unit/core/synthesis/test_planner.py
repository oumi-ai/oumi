...
class TestDatasetPlanner:
    ...
    @pytest.fixture
    def mock_permutable_attributes(self):
    ...
    def test_plan_with_no_permutable_attributes(self, planner):
        params = GeneralSynthesisParams(permutable_attributes=None)
        result = planner.plan(params, sample_count=5)
        assert isinstance(result, list)
        assert len(result) == 0

    def test_plan_with_empty_permutable_attributes(
    ...
    def test_plan_with_zero_samples(self, planner, mock_permutable_attributes):
        params = GeneralSynthesisParams(
            permutable_attributes=mock_permutable_attributes
        )
    ...
    def test_plan_with_negative_samples(self, planner, mock_permutable_attributes):
        params = GeneralSynthesisParams(
            permutable_attributes=mock_permutable_attributes
        )
    ...
    def test_plan_with_valid_permutable_attributes(
        self, planner, mock_permutable_attributes
    ):
    ...
    def test_plan_with_valid_combination_sampling(
        self, planner, mock_permutable_attributes
    ):
    ...
    def test_plan_with_multiple_combinations(self, planner, mock_permutable_attributes):
        ...
        matches = [
            sum(
                ...
                for sample in result
                if all(
                    sample.get(attr) == val for attr, val in comb.combination.items()
                )
            )
    ...
    def test_plan_resamples_on_combination_match(
        self, planner, mock_permutable_attributes
    ):
        ...
        matching_samples = sum(
            ...
            for sample in result
            if all(
                sample.get(attr) == val for attr, val in forbidden_combination.items()
            )
        )
