...
class TestQuantizerBuilders:
    ...
    @patch('oumi.builders.quantizers.AwqQuantization')
    @patch('oumi.builders.quantizers.BitsAndBytesQuantization')
    def test_build_quantizer_unsupported_method(self, mock_bnb, mock_awq, mock_get_methods):
    ...
    def test_build_quantizer_case_sensitivity(self):
        ...
        with patch('oumi.builders.quantizers.get_available_methods') as mock_get_methods:
            mock_get_methods.return_value = {"AWQ": [], "BitsAndBytes": []}
            
            with pytest.raises(ValueError, match="Unsupported quantization method: AWQ_4BIT"):
                build_quantizer("AWQ_4BIT")
    
    @patch('oumi.builders.quantizers.AwqQuantization')
    @patch('oumi.builders.quantizers.BitsAndBytesQuantization')
    def test_build_quantizer_multiple_calls_create_new_instances(self, mock_bnb, mock_awq):
