...
class TestVLLMInferenceEngine:
    def test_guided_decoding_json(self, engine, mock_sampling_params):
        """Test inference with JSON guided decoding."""
        schema = {
            "type": "object",
            "properties": {
                "name": {"type": "string"},
        ...
        conversation = Conversation(
            messages=[Message(content="Generate a person", role=Role.USER)]
        )
    ...
    def test_guided_decoding_regex(self, engine, mock_sampling_params):
        ...
        conversation = Conversation(
            messages=[Message(content="Generate a SSN", role=Role.USER)]
        )
    ...
    def test_guided_decoding_choice(self, engine, mock_sampling_params):
        """Test inference with choice guided decoding."""
        ...
        conversation = Conversation(
            messages=[Message(content="Will it rain today?", role=Role.USER)]
        )
    ...
    def test_no_guided_decoding(self, engine, mock_sampling_params):
        """Test inference without guided decoding."""
        config = InferenceConfig(
            model=ModelParams(model_name="test-model"),
            generation=GenerationParams()
        )
        ...
        # Verify SamplingParams was called without guided_decoding
        mock_sampling_params.assert_called_once()
        call_kwargs = mock_sampling_params.call_args[1]
        assert call_kwargs.get("guided_decoding") is None
