...
class TestResearchWorkflow:
    ...
    def test_document_research_journey(self):
        ...
        with temporary_chat_files(research_docs) as temp_files:
            with chat_session.real_inference_session():
                ...
                export_results = []
                export_base = Path(temp_files["paper1.txt"]).parent / "research_insights"
                ...
                successful_research_steps = sum(1 for _, success in research_results if success)
                assert successful_research_steps >= 4, f"Research workflow incomplete: {research_results}"
    ...
    def test_iterative_research_refinement(self):
        ...
        with chat_session.real_inference_session():
            ...
            assert successful_stages >= 3, f"Refinement process incomplete: {refinement_stages}"
            
            # Verify conversation progression
            conversation = chat_session.get_conversation()
            assert conversation is not None
            assert len(conversation.messages) >= successful_stages * 2  # User + Assistant pairs
...
class TestCreativeWorkflow:
    ...
    def test_creative_brainstorming_journey(self):
        ...
        with chat_session.real_inference_session():
            ...
            for topic, keyword in branch_topics:
                ...
                branch_cmd_result = chat_session.inject_command(f"/branch({topic.replace(' ', '_')})")
                ...
                if explore_result.success:
                    try:
                        chat_session.assert_response_quality(expected_keywords=[keyword])
            ...
            try:
                save_result = chat_session.inject_command(f"/save({creative_output_path})")
            ...
            assert successful_steps >= 3, f"Creative workflow incomplete: {creative_steps}"
            
            successful_branches = sum(1 for _, success in branch_results if success)
            assert successful_branches >= 2, f"Branch exploration incomplete: {branch_results}"
    ...
    def test_creative_iteration_and_refinement(self):
        ...
        with chat_session.real_inference_session():
            ...
            assert successful_iterations >= 3, f"Creative iteration incomplete: {iterations}"
...
"""
        ...
        with temporary_chat_files(analysis_data) as temp_files:
            with chat_session.real_inference_session():
                ...
                if attach_result.success:
                    data_overview_result = chat_session.send_message_with_real_inference(
                        "What patterns do you see in this sales data? "
                        "Please mention 'sales' or 'data' in your response."
                    )
                    analysis_steps.append(("data_overview", data_overview_result.success))
                ...
                if attach_result.success:
                    ...
                    analysis_steps.append(("analyze_stats", stats_analysis_result.success))
                ...
                if attach_result.success:
                    ...
                    analysis_steps.append(("integrate_notes", integrated_result.success))
                ...
                analysis_steps.append(("recommendations", recommendations_result.success))
                
                # Step 5: Create analysis report
                report_path = Path(temp_files["sales_data.csv"]).parent / "analysis_report.md"
                ...
                assert successful_steps >= 5, f"Analysis workflow incomplete: {analysis_steps}"
...
"""
        ...
        with temporary_chat_files(comparison_data) as temp_files:
            with chat_session.real_inference_session():
                ...
                # Load Q1 data
                q1_result = chat_session.inject_command(f"/attach({temp_files['q1_results.json']})")
                comparison_steps.append(("load_q1", q1_result.success))
                
                # Load Q2 data
                q2_result = chat_session.inject_command(f"/attach({temp_files['q2_results.json']})")
                ...
                if q1_result.success and q2_result.success:
                    ...
                    comparison_steps.append(("compare_quarters", comparison_result.success))
                
                # Load and apply benchmarks
                benchmark_result = chat_session.inject_command(f"/attach({temp_files['benchmark.txt']})")
                comparison_steps.append(("load_benchmark", benchmark_result.success))
                
                if benchmark_result.success:
                    benchmark_analysis_result = chat_session.send_message_with_real_inference(
                        "How do our results compare to industry benchmarks? "
                        "Please include 'benchmark' in your response."
                    )
                    comparison_steps.append(("benchmark_analysis", benchmark_analysis_result.success))
                ...
                assert successful_steps >= 4, f"Comparative analysis incomplete: {comparison_steps}"
...
"""
        ...
        with temporary_chat_files(project_data) as temp_files:
            ...
            with session1.real_inference_session():
                ...
                # Load project brief
                load_result = session1.inject_command(f"/attach({temp_files['project_brief.md']})")
                ...
                session1_save_path = Path(temp_files["project_brief.md"]).parent / "session1_state.json"
            ...
            with session2.real_inference_session():
                ...
                if session1_save_path.exists():
                    load_prev_result = session2.inject_command(f"/attach({session1_save_path})")
                ...
                # Save final state
                final_save_path = Path(temp_files["project_brief.md"]).parent / "final_project_state.json"
    ...
    def test_knowledge_building_workflow(self):
            ...
            with session.real_inference_session():
                ...
                if exploration_result.success:
                    ...
                    domain_results[domain] = {
                        ...
                        "conversation_length": len(session.get_conversation().messages) if session.get_conversation() else 0
                    }
                else:
                    domain_results[domain] = {
                        "exploration": False,
        ...
        successful_domains = sum(1 for results in domain_results.values() 
                                if results["exploration"] and results["followup"])
        assert successful_domains >= 2, f"Knowledge building incomplete: {domain_results}"
        
        # Verify each domain had meaningful conversation
        for domain, results in domain_results.items():
            if results["exploration"]:
                assert results["conversation_length"] >= 2, f"Insufficient conversation in {domain}"
    ...
    def test_multi_file_workflow_utilities(self):
        ...
        with temporary_chat_files(test_files) as temp_files:
            ...
            for original_name, temp_path in temp_files.items():
                ...
                assert original_name.split('_')[1].split('.')[0] in content  # Document number
