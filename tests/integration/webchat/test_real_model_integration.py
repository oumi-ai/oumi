...
class TestRealModelWebChatIntegration:
    ...
    def test_webchat_server_with_model_config(self, small_model_config):
        ...
        with mock_webchat_server(config=small_model_config) as server:
            ...
            assert server.config.model.model_name == "HuggingFaceTB/SmolLM-135M-Instruct"
            ...
            assert session.command_context.config.model.model_name == "HuggingFaceTB/SmolLM-135M-Instruct"
    ...
    async def test_chat_completion_via_websocket(self, mock_inference_engine):
        ...
        with patch("oumi.cli.webchat.inference_engine", mock_inference_engine):
    ...
    async def test_streaming_chat_completion(self, mock_inference_engine):
        ...
        with patch("oumi.cli.webchat.inference_engine", mock_inference_engine):
            ...
            stream_chunks = [
                ...
                {"type": "chat_stream_chunk", "delta": "This is ", "message_id": "msg_123"},
                {"type": "chat_stream_chunk", "delta": "a streaming ", "message_id": "msg_123"},
                {"type": "chat_stream_chunk", "delta": "response.", "message_id": "msg_123"},
                {
    ...
    async def test_conversation_context_management(self, mock_inference_engine):
        ...
        with patch("oumi.cli.webchat.inference_engine", mock_inference_engine):
            ...
            assert responses[0]["message"] == "Nice to meet you! I'm Claude, an AI assistant."
            assert "remember our conversation" in responses[1]["message"] or "previously mentioned" in responses[1]["message"]
            assert responses[2]["conversation_length"] == 6  # 3 user + 3 assistant messages
    ...
    def test_webchat_with_file_upload_and_model_processing(self, mock_inference_engine):
        ...
        with patch("oumi.cli.webchat.inference_engine", mock_inference_engine):
            with mock_webchat_server() as server:
                ...
                mock_file = create_mock_file_upload("ai_document.txt", test_content, "text/plain")
                ...
                session.add_message("assistant", "The document discusses AI and machine learning topics.")
    ...
    async def test_error_handling_with_model_failures(self, mock_inference_engine):
        ...
        for i, failure_response in enumerate(failure_responses):
            client.add_auto_response(f"chat_error_{i}", failure_response)
    ...
    def test_performance_monitoring_integration(self, mock_inference_engine):
        ...
        with mock_webchat_server() as server:
            ...
            session.add_message("assistant", "Machine learning is a subset of artificial intelligence...")
            
            # Test performance monitoring endpoint
            perf_response = server.handle_rest_request("GET", f"/performance/{session_id}")
...
class TestRealModelEndToEnd:
    ...
    def test_full_webchat_with_small_model(self):
        ...
        pytest.skip("Requires GPU and significant resources - run manually for full validation")
    ...
    async def test_real_websocket_with_model_inference(self):
        ...
        pytest.skip("Requires running WebChat server - integration test for CI/CD pipeline")
...
class TestModelCompatibility:
    ...
    def test_model_config_validation(self):
        ...
        for config_data in model_configs:
            ...
            with mock_webchat_server(config=config) as server:
                ...
                session = server.get_session(session_id)
                assert session.command_context.config.model.model_name == config_data["model_name"]
    ...
    async def test_model_capability_detection(self):
        ...
        assert ui_response["enabled_features"]["code_execution"] is True  # SmolLM supports code
        assert ui_response["enabled_features"]["image_upload"] is False  # No vision support