...
class TestWebSocketBasicCommunication:
    ...
    async def test_websocket_auto_responses(self):
        ...
        chat_message = {"type": "chat", "message": "Test message", "session_id": "test"}
...
class TestWebSocketMessageProtocol:
    ...
    def test_websocket_message_validation_failures(self):
        ...
        with pytest.raises(
            AssertionError, match="Message missing required field 'message'"
        ):
    ...
    def test_standard_websocket_messages(self):
        ...
        assert_websocket_message(
            test_messages["chat_message"], "chat", ["message", "session_id"]
        )

        assert_websocket_message(
            test_messages["command_message"], "command", ["command", "session_id"]
        )
        ...
        assert_websocket_message(
            test_messages["system_stats"], "system_stats", ["session_id"]
        )
    ...
    async def test_websocket_error_message_format(self):
        ...
        assert_websocket_message(
            response, "error", ["error_code", "error_message", "success"]
        )
...
class TestWebSocketSessionManagement:
    ...
    async def test_session_switching_via_websocket(self):
        ...
        switch_message = {"type": "switch_session", "session_id": "target_session_456"}
