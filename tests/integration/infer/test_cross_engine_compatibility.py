...
class TestEngineOutputConsistency:
    ...
    def test_native_vs_vllm_consistency(self):
        ...
        for result, engine_name in [(native_result, "Native"), (vllm_result, "VLLM")]:
            ...
            assert_response_relevance(
                result, expected_topics=["greeting", "hello", "conversation"]
            )
    ...
    @pytest.mark.skipif(
        not vllm_available or not llamacpp_available,
...
class TestEnginePerformanceComparison:
    ...
    def test_engine_speed_comparison(self):
        ...
        gen_params = GenerationParams(max_new_tokens=25, temperature=0.0, seed=42)
        inference_config = InferenceConfig(generation=gen_params)
        ...
        assert native_throughput > 1.0, (
            f"Native throughput too low: {native_throughput}"
        )
    ...
    def test_llamacpp_memory_vs_performance(self):
        ...
        gen_params = GenerationParams(max_new_tokens=20, temperature=0.0, seed=42)
        inference_config = InferenceConfig(generation=gen_params)
        ...
        assert throughput_no_mmap > 0.5, (
            f"No-MMAP throughput too low: {throughput_no_mmap}"
        )
    ...
    def test_gpu_vs_cpu_inference_comparison(self):
        ...
        llamacpp_throughput = measure_tokens_per_second(
            llamacpp_tokens, llamacpp_elapsed
        )
        ...
        assert llamacpp_throughput > 0.5, (
            f"LlamaCpp CPU throughput too low: {llamacpp_throughput}"
        )
...
class TestEngineFeatureCompatibility:
    ...
    def test_generation_parameter_support(self):
        ...
        parameter_sets = [
            ...
            GenerationParams(
                max_new_tokens=20, temperature=0.8, use_sampling=True, seed=42
            ),
        ]
    ...
    def test_llamacpp_hardware_parameter_support(self):
        ...
        config = InferenceConfig(
            generation=GenerationParams(max_new_tokens=10, temperature=0.0)
        )
