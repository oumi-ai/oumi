...
Follows upstream jax-llm-examples/kimi_k2/tests/test_model.py patterns.
Note: Kimi K2 does NOT use set_mesh context, and uses quantize_* naming.
"""
...
from oumi.models.experimental.jax_models.kimi_k2.kimi_k2_jax import (  # noqa: E402
    model as k2jax,
)
...
class TestKimiK2JAXIntegration:
    """Integration tests for Kimi K2 JAX model, following upstream patterns."""
    ...
    def test_cache_init(self, small_cfg):
        """KVCache init with 4 args for Kimi K2."""
        cache = k2jax.KVCache.init(random.key(0), small_cfg, 2, small_cfg.max_seq_len)
    ...
    def test_init_hashing(self, small_cfg):
        ...
        def hash_fn(x):
            return hash(tuple(jax.tree.leaves(x, is_leaf=k2jax.is_param)))

        abstract1 = k2jax.Weights.abstract(small_cfg)
        abstract2 = k2jax.Weights.abstract(small_cfg)
        assert hash_fn(abstract1) == hash_fn(abstract2)

        shardings1 = k2jax.Weights.shardings(small_cfg)
        shardings2 = k2jax.Weights.shardings(small_cfg)
        assert hash_fn(shardings1) == hash_fn(shardings2)

    def test_prefill_decode(self, small_cfg):
        """Prefill then decode without set_mesh (Kimi K2 pattern)."""
