...
from oumi.utils.cache_utils import make_hashable, dict_cache, dict_lru_cache
...
class TestDictCache:
    ...
    def test_kwargs_caching(self):
        ...
        result1 = func_with_kwargs("test", config={"a": 1}, extra={"b": 2})
        assert call_count == 1
        
        # Same args should use cache
        result2 = func_with_kwargs("test", config={"a": 1}, extra={"b": 2})
        assert call_count == 1
        
        # Different kwargs should call function
        result3 = func_with_kwargs("test", config={"a": 1}, extra={"b": 3})
        assert call_count == 2
