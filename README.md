...
Oumi provides CLI tools to help you analyze and curate your datasets for quality, safety, and composition before training or evaluation.
...
The `oumi curate` command filters and manipulates your dataset based on the analysis results. You can filter by language, length, safety, and more.

```sh
oumi curate data/analysis_results.json data/curated_dataset.jsonl --language 'en'

oumi curate data/analysis_results.json data/curated_dataset.jsonl --min_length 10 --max_length 100

oumi curate data/analysis_results.json data/curated_dataset.jsonl --max_safety_issues 1

oumi curate data/analysis_results.json data/curated_dataset.jsonl --query "machine learning" --similarity_threshold 0.3

oumi curate data/analysis_results.json data/curated_dataset.jsonl --query "machine learning" --similarity_threshold 0.3 --use_gpu True

```

- `data/analysis_results.json`: Path to the analysis results file generated by `oumi analyze`.
- `data/curated_dataset.jsonl`: Path to save the curated dataset (JSONL).
- `--min-words`, `--max-words`: Filter samples by word count.
- `--exclude-languages`: Exclude samples in specified languages (e.g., `fr` for French).
- `--exclude-safety-issues`: Exclude samples with any detected safety issues.
