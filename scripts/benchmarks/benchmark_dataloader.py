...
def main(args):
    ...
    if args.config:
        ...
        dataset_params = {
            ...
            "experimental_use_async_dataset": (
                config.data.train.experimental_use_async_dataset
            ),
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--config", type=str, help="Path to the LeMa configuration file."
    )
    parser.add_argument(
        "-d",
        "--dummy",
        action="store_true",
        help="Use a dummy dataset instead of a LeMa dataset.",
    )
    parser.add_argument(
        "--output",
        type=str,
