...
if [ "$MODEL_SIZE" == "3b" ]; then
    ...
    if [ "$DISTRIBUTION_MODE" == "ddp" ]; then
        ...
        if [ "$TRAINING_MODE" == "lora" ]; then
            ...
            if [ "$TRAINING_MODE" == "pretrain" ]; then
                ACCELERATE_CFG_FILE="configs/examples/llama3_1/pretraining/8b_full/accelerate.yaml"
                OUMI_CFG_FILE="configs/examples/llama3_1/pretraining/8b_full/train.yaml"
            fi
