...
export FRONTIER_QUEUE=batch
  ...
  # --cpu-bind=threads
  JOB_ID=$(sbatch -A lrn081 -N ${FRONTIER_NODES} -n8 -c1 --threads-per-core=1 -m "block:cyclic" -p ${FRONTIER_QUEUE} -o "/lustre/orion/lrn081/scratch/$USER/jobs/logs/job-%j.OU" -e "/lustre/orion/lrn081/scratch/$USER/jobs/logs/job-%j.ER" ${JOB_PATH})
  SBATCH_RESULT=$?
