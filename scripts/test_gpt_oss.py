...
def test_inference_config():
    ...
    config = InferenceConfig.from_yaml('configs/recipes/gpt_oss/inference/20b_vllm_infer.yaml')
    print(f"✓ 20B Config: {config.model.model_name}")
    print(f"  Engine: {config.engine}")
    print(f"  MXFP4: {config.model.model_kwargs.get('quantization_config', {}).get('quant_method')}")
    
    # Test 120B model config
    config = InferenceConfig.from_yaml('configs/recipes/gpt_oss/inference/120b_vllm_infer.yaml')
...
def test_quantization_config():
    ...
    config = QuantizationConfig.from_yaml('configs/examples/quantization/gpt_oss_mxfp4_quantization.yaml')
...
def test_engine_creation():
    ...
    try:
        ...
        if 'gpt_oss' in error_msg and 'Transformers does not recognize' in error_msg:
            ...
            return False
        elif 'Repository not found' in error_msg or 'does not appear to have a file' in error_msg:
