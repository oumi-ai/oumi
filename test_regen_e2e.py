...
def test_exact_user_sequence():
    ...
    conversation_history.extend([
        {"role": "user", "content": "Which muppet do you think would make the best president?"},
        {"role": "assistant", "content": "I think Kermit the Frog would make an excellent president! He's diplomatic, optimistic, and has experience leading a diverse group..."}
    ])
    ...
    print(f"   Before regen - last message: {conversation_history[-1]['role']}: '{conversation_history[-1]['content'][:50]}...'")
    ...
    print(f"   Has user_input_override: {hasattr(regen_result, 'user_input_override') and regen_result.user_input_override is not None}")
    
    if hasattr(regen_result, 'user_input_override') and regen_result.user_input_override:
    ...
    if conversation_history:
        print(f"   After regen - last message: {conversation_history[-1]['role']}: '{conversation_history[-1]['content'][:50]}...'")
    ...
    input_text = regen_result.user_input_override if hasattr(regen_result, 'user_input_override') else None
    ...
    print(f"   Conversation before potential user message add:")
    ...
    if not would_add_message:
        print("   ✅ GOOD: Skipping user message addition (this should prevent duplicate)")
    else:
        print("   ❌ BAD: Would add user message (this could cause role alternation error)")
    ...
    print(f"   Final conversation with potential user message:")
    ...
    for i in range(1, len(test_history)):
        if test_history[i-1]['role'] == test_history[i]['role']:
            violations.append(f"Messages {i-1} and {i} both have role '{test_history[i]['role']}'")
    
    if violations:
        print(f"   ❌ ROLE VIOLATIONS DETECTED:")
        for violation in violations:
            print(f"     - {violation}")
    else:
        print(f"   ✅ No role alternation violations")
    ...
    return {
        ...
        'has_override': hasattr(regen_result, 'user_input_override') and regen_result.user_input_override is not None,
...
if __name__ == "__main__":
    ...
    try:
        ...
        if results['regen_success'] and results['has_override'] and results['role_violations'] == 0:
