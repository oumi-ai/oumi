#!/usr/bin/env python3
"""
Script to run generated image synthesis code artifacts and create a HuggingFace dataset.
Generates PIL images from code artifacts and creates a dataset with all existing columns plus Image column.
"""
...
def resize_and_convert_image(image_path: Path, max_size: int = 1024) -> Optional[Image.Image]:
...
def preprocess_code(code: str) -> str:
    ...
    if not has_matplotlib and ('plt.' in code or 'matplotlib' in code):
        imports_to_add.append('import matplotlib\nmatplotlib.use("Agg")\nimport matplotlib.pyplot as plt')
    ...
    if 'plt.savefig' in code and 'output_image' not in code:
        code = code.replace('plt.savefig(', 'plt.savefig("output_image.png"); plt.savefig(')
...
def run_code_artifact(code: str, temp_dir: Path, sample_idx: int) -> Optional[Image.Image]:
...
def create_hf_dataset(samples: List[Dict[str, Any]], images: List[Optional[Image.Image]], images_dir: Path) -> Dataset:
    ...
    features = Features({key: Value('string') if key != 'image' else HFImage() 
                        for key in data_dict.keys()})
    
    # Handle non-string values by converting them
    for key in data_dict:
        if key != 'image':
            data_dict[key] = [str(val) if val is not None else "" for val in data_dict[key]]
...
def main():
    parser = argparse.ArgumentParser(description='Run image synthesis code and create HuggingFace dataset')
    parser.add_argument('dataset_path', type=str, help='Path to the dataset directory')
    parser.add_argument('--output-dir', type=str, help='Output directory for the HF dataset (default: dataset_path/hf_dataset)')
    ...
    parser.add_argument('--private', action='store_true', help='Make the HuggingFace repo private')
    ...
    output_dir = Path(args.output_dir) if args.output_dir else dataset_path / "hf_dataset"
