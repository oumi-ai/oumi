...
def log_gpu_memory_usage():
    ...
    if gpu_info:
        for info in gpu_info:
            logger.info(f"GPU {info['device']}: {info['allocated']:.1f}GB allocated, {info['free']:.1f}GB free, {info['total']:.1f}GB total")
...
def optimize_batch_size_for_model(model_name: str, default_batch_size: int) -> int:
    ...
    if "405b" in model_name_lower:
        ...
        return min(default_batch_size * 2, 100)  # Small models can handle larger batches
...
class CheckpointManager:
    ...
    def load_checkpoint(self) -> Tuple[int, List[Optional[str]]]:
        ...
        try:
            ...
            logger.info(f"Resuming from checkpoint: {start_index} samples already processed")
    ...
    def save_temp_results(self, dataset_dict: Dict[str, Any], artifacts: List[Optional[str]]):
        """Save temporary results that can be used to create the final dataset."""
        temp_data = []
        for i, (key, sample) in enumerate(dataset_dict.items()):
            sample_data = self._make_json_serializable(sample.copy() if isinstance(sample, dict) else {'data': sample})
            if i < len(artifacts):
...
def extract_question_from_sample(sample: Dict[str, Any]) -> str:
    ...
    text_content = sample.get('text', sample.get('prompt', sample.get('question', None)))
...
def create_image_synthesis_prompt(sample: Dict[str, Any]) -> str:
    ...
    prompt = f"""Given the following educational content from a dataset sample, create a Python code artifact that will generate or download an appropriate image to accompany this content.

DATASET SAMPLE:
{text_content}

REQUIREMENTS:
1. If the content involves mathematical concepts, formulas, or data relationships, create a matplotlib/seaborn visualization (graph, plot, diagram).
2. If the content would benefit from a real-world image (e.g., scientific phenomena, historical events, objects), write code to download an appropriate image from a reputable source.
...
IMPORTANT: Return ONLY a code artifact enclosed in ```python tags. The code should be ready to run immediately.

Consider the educational value: What visual would best help someone understand this concept?"""
...
def setup_vllm_engine(model_name: str = "meta-llama/Llama-3.2-3B-Instruct", 
    ...
    if tensor_parallel_size == -1:
        ...
        logger.warning(f"Requested tensor_parallel_size ({tensor_parallel_size}) > available GPUs ({gpu_count}). Using {gpu_count} GPUs.")
        tensor_parallel_size = gpu_count
...
def process_dataset_batch(engine, samples: List[Dict[str, Any]], 
                         generation_params: GenerationParams) -> List[Optional[str]]:
    ...
    try:
        ...
        logger.info(f"Model: {engine.model_params.model_name if hasattr(engine, 'model_params') else 'Unknown'}")
        ...
        if results and len(results) > 0:
            for i, result in enumerate(results):
                if result and result.messages:
                    ...
                    if code_artifact:
                        logger.debug(f"Sample {i}: Code artifact extracted successfully")
                    else:
                        logger.warning(f"Sample {i}: No code artifact found in response")
                else:
        ...
        if "CUDA" in error_msg or "GPU" in error_msg:
            ...
            logger.info("Consider reducing --gpu-memory-utilization or using a smaller model")
...
def main():
    parser = argparse.ArgumentParser(
        description="Synthesize images for dataset samples using local VLLM inference via Oumi"
    )
    parser.add_argument(
        "input",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output path for the synthesized dataset (defaults to local equivalent of input)"
    )
    parser.add_argument(
        "--model-name",
        type=str,
        default="meta-llama/Llama-3.2-3B-Instruct",
        help="HuggingFace model name to use for inference (default: meta-llama/Llama-3.2-3B-Instruct)"
    )
    parser.add_argument(
        "--max-samples",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Save checkpoint every N chunks when using batch processing, or every N samples otherwise (default: 10)"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=50,
        help="Number of samples to process in each batch chunk (will be auto-optimized based on model size, default: 50)"
    )
    parser.add_argument(
        "--tensor-parallel-size",
        type=int,
        default=-1,
        help="Number of GPUs to use for tensor parallelism (-1 uses all available GPUs, default: -1)"
    )
    parser.add_argument(
    ...
    optimized_batch_size = optimize_batch_size_for_model(args.model_name, args.batch_size)
    if optimized_batch_size != args.batch_size:
        logger.info(f"Optimizing batch size from {args.batch_size} to {optimized_batch_size} based on model size")
    ...
    try:
        ...
        chunk_count = (remaining_count + args.batch_size - 1) // args.batch_size  # Ceiling division
        
        logger.info(f"Processing {remaining_count} samples in {chunk_count} chunks of {args.batch_size} samples each...")
        ...
        for chunk_idx in range(chunk_count):
            ...
            logger.info(f"Processing chunk {chunk_idx + 1}/{chunk_count}: samples {start_idx}-{end_idx-1} ({actual_chunk_size} samples)")
            
            # Process this chunk
            with tqdm(total=actual_chunk_size, desc=f"Chunk {chunk_idx + 1}/{chunk_count}", leave=False) as chunk_pbar:
                try:
                    chunk_artifacts = process_dataset_batch(engine, chunk_samples, generation_params)
                    ...
                    logger.error(f"CUDA out of memory in chunk {chunk_idx + 1}. Trying to recover...")
                    torch.cuda.empty_cache()  # Clear cache
                    
                    # Try with smaller batch
                    if actual_chunk_size > 1:
                        logger.info(f"Retrying chunk {chunk_idx + 1} with smaller batch size...")
                        smaller_batches = [chunk_samples[i:i+max(1, actual_chunk_size//2)] 
                                         for i in range(0, len(chunk_samples), max(1, actual_chunk_size//2))]
                        chunk_artifacts = []
                        for small_batch in smaller_batches:
                            try:
                                small_results = process_dataset_batch(engine, small_batch, generation_params)
                        ...
                        logger.error(f"Cannot reduce batch size further for chunk {chunk_idx + 1}")
            ...
            if (chunk_idx + 1) % args.save_interval == 0 or chunk_idx == chunk_count - 1:
                checkpoint_manager.save_checkpoint(end_idx - 1, code_artifacts[:end_idx])
                
                # Also save temporary results
                df = dataset.select(range(end_idx)).to_pandas()
                dataset_dict = {}
                for idx, row in df.iterrows():
                    dataset_dict[str(idx)] = _convert_to_json_serializable(row.to_dict())
                checkpoint_manager.save_temp_results(
                    dataset_dict, 
                    code_artifacts[:end_idx]
                )
                logger.info(f"Progress saved: {end_idx}/{total_samples} samples processed ({chunk_idx + 1}/{chunk_count} chunks)")
        
        logger.info(f"Batch processing complete: {total_samples}/{total_samples} samples processed in {chunk_count} chunks")
        ...
        if last_completed > 0:
            checkpoint_manager.save_checkpoint(last_completed - 1, code_artifacts[:last_completed])
        raise
    except Exception as e:
    ...
    df['image_synthesis_code'] = code_artifacts[:len(df)]
    
    # Convert back to dataset
    enhanced_dataset = Dataset.from_pandas(df)
    ...
    logger.info(f"Successfully generated {successful_artifacts}/{len(dataset)} code artifacts")
    ...
    if successful_artifacts < len(dataset):
        ...
        logger.warning(f"{failed_count} samples failed to generate code artifacts. Check logs for details.")
...
def test_vllm_setup(model_name: str = "meta-llama/Llama-3.2-3B-Instruct"):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        model_name = sys.argv[2] if len(sys.argv) > 2 else "meta-llama/Llama-3.2-3B-Instruct"
